...
namespace ost {
...
#if 0
        {
            STATUS("Waiting for thread finish mutex");
            ost::MutexLock lock(thread.execution_status_mutex);
            STATUS("Got thread finish mutex for " << &thread);
            thread.running = false;
            thread.stopped_execution.signal();
            STATUS("Detaching " << &thread);
    ...
    void Thread::callFinal(void *threadp) throw() { 
        Thread &thread = *(Thread*)threadp;
        STATUS("Finalizing " << ((thread.detached) ? "detached " : "")
               << "thread " << threadp);
        thread.final(); 
        if ( thread.detached ) { 
            delete &thread;
    ...
    void Thread::join() throw() {
        STATUS("Calling join on thread " << this);
        if (running && !detached)  {
            STATUS("Joining thread " << this);
#if 0
            STATUS("Waiting for thread finish mutex for " << this);
            ost::MutexLock lock( execution_status_mutex );
            STATUS("Got thread finish mutex for" << this);
            while ( running ) {
                STATUS("Waiting for execution to stop for " << this);
