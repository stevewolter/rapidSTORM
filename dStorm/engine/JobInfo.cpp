...
{
    assert( (*traits.plane(plane).optics.psf_size(0))[dimension] > 0 * boost::units::si::meter );
    ...
    boost::units::quantity< boost::units::si::length > max_sigma =
        (*traits.plane(plane).optics.psf_size(0))[dimension] * max_wavelength / traits.fluorophores.at(0).wavelength;
