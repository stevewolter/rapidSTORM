...
{
    assert( (*traits.plane(0).psf_size(0))[dimension] > 0 * boost::units::si::meter );
    ...
    boost::units::quantity< boost::units::si::length > max_sigma =
        (*traits.plane(0).psf_size(0))[dimension] * max_wavelength / traits.fluorophores.at(0).wavelength;
...
boost::units::quantity< boost::units::si::length > JobInfo::sigma_in_si( int dimension, int plane ) const
{
    ...
    boost::units::quantity< boost::units::si::length > sigma =
        (*traits.plane(0).psf_size(0))[dimension] * factor;
