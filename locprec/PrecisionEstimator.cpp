...
#include <sstream>
#include <dStorm/statistics/Variance.h>
...

{
    ...
    double *values = new double[ trace.size() ],
           *weights = new double[ trace.size() ];
    int j = 0;
    for ( dStorm::Trace::const_iterator i = trace.begin();
          i != trace.end(); i++, j++)
    {
        values[j] = (coordinate == 0) ? i->x() : i->y();
        weights[j] = i->getStrength();
    }

    double sd = gsl_stats_wsd(weights, 1, values, 1, j);
    delete[] values;
    delete[] weights;
    return sd;
}
