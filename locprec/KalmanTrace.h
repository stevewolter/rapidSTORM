...
class KalmanTrace : public dStorm::Trace {
    ...
    struct Observation : public Eigen::Matrix<double,Obs,1> {
        Observation(const dStorm::Localization& l) 
            : Eigen::Matrix<double,Obs,1>(l.x(), l.y()) {}
    };
...
; 
    ...
    double getEstimate_X() const { return position_estimate[0]; }
    double getEstimate_Y() const { return position_estimate[1]; }
};
...
{
    update_prediction( position.getImageNumber() );
...
void KalmanTrace<Dimensions>::add( const dStorm::Localization& l )
{
    ...
    if ( previous_observations_available )
        update_position( obs, l.getImageNumber() );
    else {
        position_estimate = observation_matrix.transpose() * obs;
        position = l.getImageNumber();
    }
#if 0
    std::cout << "1 " 
        << myNumber << " "
