  ...
  public:
    _Config() 
        : simparm::Object("RipleyK", "Compute Ripley's K function"),
          bin_size("BinSize", "Bin size", 1 * si::nanometre) {}
    void registerNamedEntries() { push_back( bin_size ); }
...
Output::AdditionalData Output::announceStormSize(const Announcement& a) {
    ...
    measured_area = quantity<si::length>::from_value(scalers[0].get_size()) * 
                    quantity<si::length>::from_value(scalers[1].get_size());
...
void Output::propagate_signal(ProgressSignal e) {
    if ( e == Engine_is_restarted && histogram.is_initialized() ) {
        ...
        quantity< boost::units::power_typeof_helper< si::area, boost::units::static_rational<-1> >::type >
        ...
        for (unsigned int i = 0; i < h.counts.size(); ++i) {
            quantity<si::length> bin_position 
                = quantity<si::length>::from_value( scalers[0].reverse_mapping( h.bin_positions[i] ) );
            quantity<si::area> ring_area = area_of_ring( bin_position, bin_size );
            std::cout << bin_position.value() << "\t" 
                      << ring_area.value() << "\t"
                      << (h.counts[i] * 2) << "\t"
...
{
    return std::auto_ptr<OutputSource>( new dStorm::output::OutputBuilder<ripley_k::Output>() );
}
