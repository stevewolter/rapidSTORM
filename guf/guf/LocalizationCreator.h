...
class LocalizationCreator {
    ...
    void write_parameters( Localization& loc, const FittedPlane&, double chi_sq, const FittingRegion& data ) const;
    void join_localizations( Localization& into, const std::vector<Localization>&, bool ) const;
    void compute_uncertainty( Localization& loc, const FittedPlane&, const FittingRegion& data ) const;

  public:
    LocalizationCreator( const Config& config, const dStorm::engine::JobInfo& optics );

    void operator()( Localization& loc, const FitPosition&, double chi_sq, const FittingRegionStack& ) const;
