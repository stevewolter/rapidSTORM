...
class LocalizationCreator {
    ...
    void write_parameters( Localization& loc, const MultiKernelModel&, double chi_sq, const fit_window::FittingRegion& data ) const;
    void join_localizations( Localization& into, const std::vector<Localization>&, bool ) const;
    void compute_uncertainty( Localization& loc, const MultiKernelModel&, const fit_window::FittingRegion& data ) const;

  public:
    LocalizationCreator( const Config& config, const dStorm::engine::JobInfo& optics );

    void operator()( Localization& loc, const MultiKernelModelStack&, double chi_sq, const fit_window::FittingRegionStack& ) const;
