...
int Source::dimx() const { 
    /* The width may change once we get on the camera (for example,
     * small detector), therefore we must wait here. */
    waitForInitialization();
    return width; 
}
int Source::dimy() const {
    /* The height may change once we get on the camera (for example,
     * small detector), therefore we must wait here. */
    waitForInitialization();
    return height; 
}
...
CImgBuffer::AndorDirect::Config::Config(const Config &c, 
...
CImgBuffer::AndorDirect::Config::~Config() {
    switcher.reset(NULL);
}
...
void CImgBuffer::AndorDirect::Config::registerNamedEntries()
