...
class ConfigEntryChoice extends ConfigEntryChooser {
        class Choice implements ConfigObject.Listener {
            ...
            private boolean madeUserLevel = false, mayBeVisible = false,
                            wasAdded = false;
            ...
            public void setRequiredUserLevel(int requiredLevel) {
                madeUserLevel = ( requiredLevel >= userLevel );
                checkAddition();
            }
            private void checkAddition() {
                boolean shouldBeAdded = madeUserLevel && node.isDirectlyViewable();
                if ( shouldBeAdded && !wasAdded )
                    addChoice( name, this );
                else if ( !shouldBeAdded && wasAdded )
                    removeChoice( name );
                wasAdded = shouldBeAdded;
            }
	...
	public void setValue(Object newvalue, boolean doprint,
        ...
        public boolean processAttribute(String name, String val)
            throws IOException 
        {
            if ( isValue(name) )
                setValueFromWord( val );
            else
                return super.processAttribute(name, val);
