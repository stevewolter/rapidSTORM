...
public class Main {
    public final static void main(String args[]) throws IOException {
        final JFrame f = new JFrame("Twiddler");

        final Twiddler twiddler;
        if (args.length > 0) {
            twiddler = new Twiddler(args);
        } else {
        ...
        LinkedList<JMenu> menus = twiddler.makeMenus();
        for ( JMenu menu : menus )
            menuBar.add(menu);
        f.setJMenuBar(menuBar);

        f.addWindowListener(new WindowAdapter() {
            private boolean was_closed;
